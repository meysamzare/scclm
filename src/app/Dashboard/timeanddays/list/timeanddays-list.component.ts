...
@Component({
    templateUrl: "./timeanddays-list.component.html",
    styles: [
        `
...
export class TimeAndDaysListComponent{
    ...
    dataSource: MatTableDataSource<ITimeAndDays>;
    selection = new SelectionModel<ITimeAndDays>(true, []);
    ...
    getCurrentDataOfPage(): ITimeAndDays[] {
        let List: ITimeAndDays[];
    ...
    deleteSelected() {
        if (this.auth.isUserAccess("remove_TimeandDays")) {
            if (this.selection.selected.length != 0) {
                ...
                this.auth.post("/api/TimeAndDays/Delete", ids, {
                    type: 'Delete',
    ...
    onEdit(id) {
        if (this.auth.isUserAccess("edit_TimeandDays")) {
            this.router.navigate(["/dashboard/dayofweek/edit/" + id]);
        }
    }
            ...
            .subscribe(
                (data: jsondata) => {
                    if (data.success) {
                        ...
                        this.dataSource = new MatTableDataSource(this.TimeAndDays);
                    } else {
                        this.isLoadingResults = false;
                        this.message.showMessageforFalseResult(data);
                    }
