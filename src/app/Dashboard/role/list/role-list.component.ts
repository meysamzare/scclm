...
export class RoleListComponent implements OnInit, AfterViewInit {
    ...
    dataSource: MatTableDataSource<RoleClass>;
    selection = new SelectionModel<RoleClass>(true, []);
    ...
    getCurrentDataOfPage(): RoleClass[] {
        let List: RoleClass[];
    ...
    deleteSelected() {
        if (this.auth.isUserAccess("remove_Role")) {
            if (this.selection.selected.length != 0) {
                ...
                this.auth.post("/api/Role/Delete", ids, {
                    type: 'Delete',
    ...
    onEdit(id) {
        if (this.auth.isUserAccess("edit_Role")) {
            this.router.navigate(["/dashboard/role/edit/" + id]);
        }
    }
