...
export class InsuranceListComponent {
    ...
    dataSource: MatTableDataSource<IInsurance>;
    selection = new SelectionModel<IInsurance>(true, []);
    ...
    getCurrentDataOfPage(): IInsurance[] {
        let List: IInsurance[];
    ...
    deleteSelected() {
        if (this.auth.isUserAccess("remove_Insurance")) {
            if (this.selection.selected.length != 0) {
                ...
                this.auth.post("/api/Insurance/Delete", ids, {
                    type: 'Delete',
    ...
    onEdit(id) {
        if (this.auth.isUserAccess("edit_Insurance")) {
            this.router.navigate(["/dashboard/insurance/edit/" + id]);
        }
    }
