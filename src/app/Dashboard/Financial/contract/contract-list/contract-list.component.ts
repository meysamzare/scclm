...
export class ContractListComponent implements OnInit {
    ...
    constructor(
        ...
        private printSrv: PrintDataService
    ) { }
    ...
    getCurrentDataOfPage(): IContract[] {
        let List: IContract[];
    ...
    deleteSelected() {
        if (this.auth.isUserAccess("remove_Contract")) {
            if (this.selection.selected.length != 0) {
                ...
                this.auth.post("/api/Contract/Delete", ids, {
                    type: 'Delete',
    ...
    onEdit(id) {
        if (this.auth.isUserAccess("edit_Contract")) {
            this.router.navigate(["/dashboard/contract/edit/" + id]);
        }
    }
    ...
    printContract(Id) {
        this.auth.post("/api/Contract/getPrintContent", Id, {
            type: 'View',
            agentId: this.auth.getUserId(),
            agentType: 'User',
            agentName: this.auth.getUser().fullName,
            tableName: 'Print Contract',
            logSource: 'dashboard',
            object: {
                contractId: Id
            },
