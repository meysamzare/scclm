...
import { IProduct, getProductTypeString } from '../product';
...
export class ProductListComponent implements OnInit {
    ...
    constructor(
        ...
        this.activeroute.data.subscribe(data => {
            ...
            if (this.TYPE == 1) {
                ...
                this.PAGE_ROLE = "VirtualTeaching";
    ...
    getTypeString(type, value): string {
        var typeString = getProductTypeString(type);

        if (type == 0 || type == 1) {
            return `${typeString} | ${value} صفحه`;
        }

        if (type == 2) {
            return `${typeString} | ${value} دقیقه`;
        }
    }
